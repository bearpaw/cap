...
def setLayers(data_source, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=False, num_output=7):
    ...
    if deploy == False:
        ...
        n.tops[label_name[1]], n.tops[label_name[0]] = L.Slice(n.label, slice_param=dict(axis=1, slice_point=num_output), ntop=2)
    else:
        input = "data"
        dim1 = 1
        dim2 = 4
        ...
        if layername[l] == 'C':
            if state == 'image':
                if layername[l+1] != 'L':
                    ...
                    conv_name = 'Fconv%d_stage%d' % (conv_counter, stage)
            else:
                conv_name = '20160601Mconv%d_stage%d' % (conv_counter, stage)
            if stage == 1:
                lr_m = 1
            else:
                lr_m = 1
...
def writePrototxts(dataFolder, dir, batch_size, stepsize, layername, kernel, stride, outCH, transform_param_in, base_lr, folder_name, label_name, dbname, num_output=7):
    ...
    with open('%s/pose_train_test.prototxt' % dir, 'w') as f:
        ...
        str_to_write = setLayers(dataFolder, batch_size, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=False, num_output=num_output)
        f.write(str_to_write)

    with open('%s/pose_deploy.prototxt' % dir, 'w') as f:
        print 'writing %s/pose_deploy.prototxt' % dir
        str_to_write = str(setLayers('', 0, layername, kernel, stride, outCH, label_name, transform_param_in, deploy=True, num_output=num_output))
        f.write(str_to_write)

    with open('%s/pose_solver.prototxt' % dir, "w") as f:
        solver_string = getSolverPrototxt(base_lr, folder_name, stepsize, dbname)
        print 'writing %s/pose_solver.prototxt' % dir
        f.write('%s' % solver_string)


def getSolverPrototxt(base_lr, folder_name, stepsize, dbname):
    string = 'net: "prototxt/%s/pose_train_test.prototxt"\n\
...
solver_mode: GPU\n' % (dbname, base_lr, stepsize, folder_name)
    return string
...
if __name__ == "__main__":
    ...
    # dbname = 'shortPin'
    num_output = 8
    if not os.path.exists('prototxt/'+dbname):
        os.makedirs('prototxt/'+dbname)
    # write training scripts
    filename = 'prototxt/'+dbname+'/train.sh'
    trainfile = open(filename, 'w')
    cmd = 'postfix=`date +"%m_%d_%y"`\n'
    cmd += '%s/build/tools/caffe train \\\n' % caffe_path[0].rstrip()
    cmd += '-solver ./prototxt/'+dbname+'/pose_solver.prototxt \\\n'
    cmd += '-weights ./prototxt/shortPin_longPin_all/caffemodel/pose_iter_27000.caffemodel \\\n'
    cmd += '-gpu 6,7  2>&1 | tee ./prototxt/'+dbname+'/caffemodel/train_$postfix.log'
    trainfile.write(cmd)

    # generate protofiles
    ### Change here for different dataset
    directory = 'prototxt/'+dbname
    dataFolder = './cache/lmdb/'+ dbname
    ...
    transform_param = dict(stride=8, crop_size_x=368, crop_size_y=368, 
                             target_dist=160*368/1200, scale_prob=1, scale_min=0.7, scale_max=1.3,
                             max_rotate_degree=360, center_perterb_max=0, do_clahe=False, num_parts=num_output-1, visualize=False)
    nCP = 3
    CH = 128
    stage = 6
    ...
    outCH =     [128, 128] * nCP + [ 32,512, 0 ,512, 0 ,num_output ] + [0] # first-stage
    stride =    [ 1 ,  2 ] * nCP + [ 1 , 1 , 0 , 1 , 0 , 1 ] + [0] # first-stage

    if stage >= 2:
        layername += ['C', 'P'] * nCP + ['$'] + ['C'] + ['@'] + ['C'] * 5            + ['L']
        outCH +=     [128, 128] * nCP + [ 0 ] + [32 ] + [ 0 ] + [128,128,128,128,num_output] + [ 0 ]
        kernel +=    [ 9,   3 ] * nCP + [ 0 ] + [ 5 ] + [ 0 ] + [11, 11, 11, 1,   1] + [ 0 ]
        stride +=    [ 1 ,  2 ] * nCP + [ 0 ] + [ 1 ] + [ 0 ] + [ 1 ] * 5            + [ 0 ]

        for s in range(3, stage+1):
            layername += ['$'] + ['C'] + ['@'] + ['C'] * 5            + ['L']
            outCH +=     [ 0 ] + [32 ] + [ 0 ] + [128,128,128,128,num_output] + [ 0 ]
            kernel +=    [ 0 ] + [ 5 ] + [ 0 ] + [11, 11, 11,  1, 1 ] + [ 0 ]
            stride +=    [ 0 ] + [ 1 ] + [ 0 ] + [ 1 ] * 5            + [ 0 ]

    label_name = ['label_1st_lower', 'label_lower']
    writePrototxts(dataFolder, directory, batch_size, stepsize, layername, kernel, stride, outCH, transform_param, base_lr, d_caffemodel, label_name, dbname, num_output)