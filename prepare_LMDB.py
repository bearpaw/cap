...
def writeLMDB(datasets, lmdb_path, json_path, path_header, validation):
	...
	for d in range(len(datasets)):
		print datasets[d]
		with open(json_path[d]) as data_file:
			data_this = json.load(data_file)
			data_this = data_this['root']
			data = data + data_this
		numSample = len(data)
		print numSample
		data_idx = data_idx + [d]*numSample
	...
	for count in range(numSample):
		...
		img = cv2.imread(os.path.join(path_header[data_idx[idx]], data[idx]['img_paths']))
		height = img.shape[0]
		width = img.shape[1]
		if(width < 64):
...
if __name__ == "__main__":
	if not os.path.exists('cache/lmdb'):
		os.makedirs('cache/lmdb')

	# You can merge multiple dataset into a single LMDB file
	json_path = ['cache/json/shortPin.json', 'cache/json/longPin.json', 'cache/json/20160601.json']
	path_header = ['/home/wyang/code/robotics/cap/dataset/shortPin/originImg', '/home/wyang/code/robotics/cap/dataset/longPin/originImg','/home/wyang/code/robotics/cap/dataset/20160601/originImg']
	datasets = ['shortPin', 'longPin', '20160601']
	lmdb_path = 'cache/lmdb/shortPin_longPin_all'
	writeLMDB(datasets=datasets, lmdb_path=lmdb_path, json_path=json_path, path_header=path_header, validation=0) # joint dataset
