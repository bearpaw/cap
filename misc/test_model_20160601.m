...
for i = 6:testlen
  ...
  for j = 1:np
    ...
    num_det = size(peaks, 2);
    bbox_center = [x1+x2; y1+y2]*0.5;
    for obj = 1:num_det
      mindist = 1e10;
      for peakid = 1:size(peaks, 2)
        curdist = norm( peaks(:, peakid) - bbox_center);
        if curdist < mindist
          mindist = curdist;
          minidx(j) = peakid;
        end
    ...
    if 0%makeFigure %&& j == 3
      ...
      plot_box(peaks(:, minidx(j)), 100);  % plot bounding box
  ...
  imToShow = single(visImg)/255 * 0.5 + mat2im(final_score(:,:,j), jet(100), [0 max_value])/2;
  rectpos = peaks(:, minidx(2:5));
  leftpin = peaks(:, minidx(1))';
  rightpin = peaks(:, minidx(6))';
  ...
  imToShow = insertShape(imToShow, 'FilledCircle', [leftpin 6], 'Color', colors(obj, :), 'Opacity', 0.8);
  imToShow = insertShape(imToShow, 'FilledCircle', [rightpin 6], 'Color', colors(obj, :), 'Opacity', 0.8);
  imshow(imToShow); hold on;
  plot([x1 x1 x2 x2 x1], [y1 y2 y2 y1 y1], 'w-.', 'LineWidth', 5); hold on;
  pause;
  %   elapse = toc;
