...
% dbname = 'shortPin';
dbname = 'longPin';
datadir = ['./dataset/' dbname '/'];

makeFigure = 1;
colors = colormap(lines(20));

cachedir = sprintf('./cache/%s/joint_pred/', dbname);
...
joint_all = cell(length(imgnames), 1);
for i = 1:length(imgnames)
  ...
  for j = 1:np
    ...
    if makeFigure %&& j == 3
      ...
      imwrite(imToShow, sprintf([cachedir '/%s_%.2d.jpg'], imageName, j));
  ...
  if makeFigure %&& j == 3
    ...
    imwrite(imToShow, sprintf([cachedir '%s_%.2d_visualize.jpg'], imageName, j));
